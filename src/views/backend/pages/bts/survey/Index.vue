      ...
      >
        <v-card>
          ...
          >
            ...
            >mdi-circle</v-icon> Formulir Survey BTS
          </v-toolbar>
          <v-card-text class="mt-2">
            ...
            <v-col cols="12">
              <v-select
                ...
                hide-details
                :color="theme.color"
                v-model="record.perusahaan_uuid"
                :items="perusahaans"
              ></v-select>
            </v-col>
            <v-col cols="12">
            ...
            <v-row class="justify-center mb-5 mt-5 title">PETA LOKASI BTS</v-row>
            <v-col>
              <v-img
                height="100%"
                width="100%"
        ...
        <v-card>
          ...
          >
            <v-icon
              ...
              color="orange"
              class="mr-1 animate__animated animate__flash animate__infinite"
            >mdi-circle</v-icon> Formulir Import Data BTS
          </v-toolbar>
          <v-card-text>
            <v-col cols="12">
              <v-select
            ...
            <v-col cols="12">
              <v-text-field
                ...
                placeholder=""
                dense
                @click:append-outer="uploadFileBts"
                hide-details
              ></v-text-field>
            </v-col>
          </v-card-text>
...
export default {
  ...
  data: () => ({
    ...
    attribution:
      '&copy; <a target="_blank" href="http://osm.org/copyright">DISKOMINFO PROV. BANTEN</a> ',
    zoom: 13,
    ...
    marker: {
      ...
      tooltip: "Pilih lokasi bts/menara",
      draggable: true,
      visible: true,
    },
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/v2/bts/survey",
      pagination: false,
      key: "id",
      title: "DATA SURVEY",
      subtitle: "Berikut Data Seluruh Survey Yang Tersedia",
      breadcrumbs: [
        {
          text: "BTS",
          disabled: true,
  ...
  methods: {
    ...
    fetchPerusahaans: async function () {
      try {
        let { data } = await this.http.get("api/v2/bts/combo/perusahaan");
        this.perusahaans = data;
      } catch (error) {}
    },
    ...
    postImportBts: async function () {
      try {
        this.setLoading({ dialog: true, text: "Proses import data bts" });
        let {
          data: { code, success, message, errors },
        } = await this.http.post("api/v2/bts/import-data", this.record);
        if (!success) {

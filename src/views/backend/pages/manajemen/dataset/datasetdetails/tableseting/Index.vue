      ...
      <v-col cols="12">
        <v-card class="animated animate__fadeInUp rounded-0">
            ...
            >
              <template v-slot:activator="{ on }">
                ...
                >
                  <v-icon
                    :color="theme.mode == 'dark' ? `white` : `black`"
                    @click="$router.push({name:'manajemen-dataset-detail',params:{dataset_uuid: $route.params.dataset_uuid}})"
                  >mdi-close-circle</v-icon>
                </v-btn>
              </template>
              <span>Kembali</span>
          ...
          >
            ...
            <template v-slot:item.sortable="{ value }">
              <v-chip
                :color="value.color"
                small
                class="white--text"
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl:
        "api/v2/manajement/dataset-detail-row-table-setting/" +
        this.$route.params.dataset_uuid,
      pagination: false,
      key: "id",
      title: "PENGATURAN TABEL",
      subtitle: "Berikut Seluruh Pengatuan Tabel  Yang Tersedia",
      breadcrumbs: [
        {
          text: "Detail Dataset",
          disabled: true,

        ...
        <v-card class="animated animate__fadeInUp rounded-0">
          <v-card-title :class="`flex flex-row-reverse ` + theme.color + ` lighten-1`">
              ...
              <template v-slot:activator="{ on }">
                ...
                >
                  <v-icon
                    ...
                    @click="$router.push({name:'manajemen-dataset'})"
            ...
            >
              <template v-slot:activator="{ on }">
                ...
                >
                  <v-icon
                    :color="theme.mode == 'dark' ? `white` : `black`"
                    @click="openDataseTableSeting($route.params.dataset_uuid)"
            ...
            >
              ...
              <span>Tambah API Integrasi</span>
            </v-tooltip>
            <v-tooltip
              :color="theme.color"
              bottom
          ...
          >
            ...
            <template v-slot:item.id="{ value }">
              ...
              >
                ...
                <v-list>
                  <v-list-item
                    @click="openDataseTableSeting(value)"
                    v-show="false"
                  >
                    <v-list-item-title>
      ...
      >
        <v-card>
          ...
          <v-card-text class="mt-2">
            ...
            <v-col cols="12">
              <span class="subtitle-1 font-weight-bold">API SERVICE</span>
            </v-col>
            <v-col cols="12">
              <v-text-field
                label="URL API SERVICE"
                placeholder="URL"
                outlined
                dense
                hide-details
                v-model="record.apipath"
              ></v-text-field>
            </v-col>
            <v-col cols="12">
              <v-text-field
        ...
        <v-card>
          ...
          >
            <v-icon
              ...
              color="orange"
              class="mr-1 animate__animated animate__flash animate__infinite"
            >mdi-circle</v-icon> Formulir Import Dataset
          </v-toolbar>
          <v-card-text class="mt-2">
            <v-col cols="12">
              <v-text-field
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl:
        "api/v2/manajement/dataset-details/" +
        this.$route.params.dataset_uuid +
        "/" +
        this.$route.params.method,
      pagination: false,
      key: "id",
      title: "MANAJEMEN DETAIL DATASET",
      subtitle: "Berikut Daftar Seluruh Detail Data Yang Tersedia",
      breadcrumbs: [
        {
          text: "Manajemen",
          disabled: true,
  ...
  methods: {
    ...
    initHeader: async function () {
      try {
        let { data } = await this.http.get(
          "api/v2/manajement/dataset-details-init-headers/" +
            this.$route.params.dataset_uuid
        );
        this.headers = data;
      } catch (error) {}
    },
    reInitHeader: async function () {
      await this.initHeader().then(() => {
        this.fetchRecords();
      });
    },
    ...
    openImportDataSet: function () {
      this.importdata.form.show = true;
    },
    closeImportDataSet: function () {
      this.filename = null;
      this.importdata.form.show = false;
    },
    openDataseTableSeting: function (val) {
      this.$router.push({
        name: "manajemen-dataset-detail-table-seting",
        params: {
          dataset_uuid: this.$route.params.dataset_uuid,
          methode: this.$route.params.method,
        },
      });
    },
    openDatasetRow: function (val) {
      this.$router.push({
        name: "manajemen-dataset-detail-row",
        params: {
          dataset_uuid: this.$route.params.dataset_uuid,
          dataset_detail_uuid: val,
        },
      });
    },
    postImport: async function () {
      try {
        ...
        let {
          data: { success, code, data, message },
        } = await this.http.post(
          "api/v2/manajement/dataset-details-import/" +
            this.$route.params.dataset_uuid,
          this.record
        );
        if (!success) {
